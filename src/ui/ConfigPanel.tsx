...
export function ConfigPanel({cfg, setCfg}:{cfg: SimConfig, setCfg: (c:SimConfig)=>void}){
  ...
  return (
    <div>
      ...
      <Row label="Long Distance">{num('longDistance', 0, 10, 1)}</Row>
      <Row label="Prestige Threshold">{num('prestigeThreshold', 0, 1, 0.01)}</Row>
      <p style={{color:'#9aa4af', fontSize:12, marginTop:12}}>Changes apply live. Use "Reset" to regenerate with new world parameters.</p>
    </div>
  )
}
